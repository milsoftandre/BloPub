                            ...
                            @endforeach
                                ...
                                <div class="col-md-3 right" style="text-align: right;">
                                    <div class="btn-group" role="group" aria-label="Basic example">
                                        <a href="{{ route('employee.index') }}" class="btn btn-danger btn-sm"> {{__('employee.bc')}} </a><button type="submit" class="btn btn-primary btn-sm">
                                            <span class="indicator-label"> {{__('employee.bs')}} </span></span>
                                        </button>
                                        @if ($settings['isAdd'])
                                            <a href="{{ route('employee.create') }}" class="btn btn-warning btn-sm">{{ $settings['buttons']['add'] }}</a>
                                        @endif
                                    </div>
                        ...
                        <div class="table-responsive">
                            <table class="table align-middle mb-0">
                                ...
                                <tr class="">
                                    ...
                                    @endforeach
                                    <th class="min-w-100px text-end">{{__('employee.act')}}</th>
                                </tr>
                                </thead>
                                <tbody>
                                @foreach ($rows as $row)
                                <tr>
                                    ...
                                    <td class="text-end">
                                        <form action="{{ route('employee.destroy',$row->id) }}" method="POST">
                                            @if ($settings['isShow'])
                                        <a href="{{ route('employee.show',$row->id) }}" class="btn btn-light">
                                            <i class="fas fa-file"></i>
                                        </a>
                                            @endif
                                                @if ($settings['isEdit'])
                                        <a href="{{ route('employee.edit',$row->id) }}" class="btn btn-light">
