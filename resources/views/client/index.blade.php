                            ...
                            <div class="row">
                                ...
                                <div class="col-md-3 right" style="text-align: right;">
                                    <div class="btn-group" role="group" aria-label="Basic example">
                                        <a href="{{ route('client.index') }}" class="btn btn-danger btn-sm"> {{__('client.bc')}} </a><button type="submit" class="btn btn-primary btn-sm">
                                            <span class="indicator-label"> {{__('client.bs')}} </span></span>
                                        </button>
                                        @if ($settings['isAdd'])
                                            <a href="{{ route('client.create') }}" class="btn btn-warning btn-sm">{{ $settings['buttons']['add'] }}</a>
                                        @endif
                                    </div>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            <form action="{{ route('client.destroy',$row->id) }}" method="POST">
                                                @if ($settings['isShow'])
                                                    <a href="{{ route('client.show',$row->id) }}" class="btn btn-light">
                                                        <i class="fas fa-file"></i>
                                                    </a>
                                                @endif
                                                @if ($settings['isEdit'])
                                                    <a href="{{ route('client.edit',$row->id) }}" class="btn btn-light">
