            ...
            <div id="kt_content_container" class="container-xxl">
                <div class="card mb-5 mb-xl-8">
                    <div class="card-header border-0">
                        ...
                        <div class="col-md-12">
                            {{ Form::open(['route' => ['group.index'],'method' => 'get']) }}
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-3 right" style="text-align: right;">
                                    <div class="btn-group" role="group" aria-label="Basic example">
                                        <a href="{{ route('group.index') }}" class="btn btn-danger btn-sm"> {{__('group.bc')}} </a><button type="submit" class="btn btn-primary btn-sm">
                                            <span class="indicator-label"> {{__('group.bs')}} </span></span>
                                        </button>
                                        @if ($settings['isAdd'])
                                            <a href="{{ route('group.create') }}" class="btn btn-warning btn-sm">{{ $settings['buttons']['add'] }}</a>
                                        @endif
                                    </div>
                        ...
                        <div class="table-responsive">
                            <table class="table align-middle mb-0">
                                ...
                                <tr class="">
                                    ...
                                    @endforeach
                                    <th class="min-w-100px text-end">{{__('group.act')}}</th>
                                </tr>
                                </thead>
                                <tbody>
                                @foreach ($rows as $row)
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            <form action="{{ route('group.destroy',$row->id) }}" method="POST">
                                                @if ($settings['isShow'])
                                                    <a href="{{ route('group.show',$row->id) }}" class="btn btn-light">
                                                        <i class="fas fa-file"></i>
                                                    </a>
                                                @endif
                                                @if ($settings['isEdit'])
                                                    <a href="{{ route('group.edit',$row->id) }}" class="btn btn-light">
