        ...
        <div class="post d-flex flex-column-fluid" id="kt_post">
            ...
            <div id="kt_content_container" class="container-xxl">
                ...
                <div class="card mb-5 mb-xl-8">
                    <div class="card-header border-0"><div class="card-title m-0"><h3 class="fw-bolder m-0">{{__('hand.edit')}}</h3></div></div>
                    {{ Form::open(['route' => ['hand.update',$model->id]]) }}
                        ...
                        @foreach ($settings['form'] as $fname => $field)
                            <div class="row mb-6 pt-2 divfield divfield_{{$fname}}">
                                ...
                                <div class="col-lg-8 fv-row">
                                    @if($field=='text')
                                        {{ Form::text($fname, $model->$fname, ['placeholder'=> $settings['attr'][$fname], 'class'=>'form-control form-control-lg form-control-solid inpfield_'.$fname]) }}
                                    @endif
                                    @if($field=='password')
                    ...
                    <div class="card-footer d-flex justify-content-end py-6 px-9">
                        <a href="{{ route('hand.index') }}" class="btn btn-white btn-active-light-primary me-2"> {{__('hand.edit_cansel')}} </a>
                        <button type="submit" id="kt_account_profile_details_submit" class="btn btn-primary">
                            <span class="indicator-label"> {{__('hand.edit')}} </span></span>
                        </button>
                    </div>
                    {{ Form::close() }}
                </div>
